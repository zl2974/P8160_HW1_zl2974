...
newton_update =
  function(fun,
    ...
    #backtracking
    if (abs(fun(data,previous_theta)$loglink) == Inf) stop("Check your log-likelihood")
    trial = 0
    
    gradient = fun(data,previous_theta)$gradient
    
    if (is.function(optimizer)) {
      hessian = optimizer(data, fun,) # get H
    } else{
      if (is.numeric(optimizer)) {
        H = optimizer # use H
      } else{
        hessian = fun(data, previous_theta)$hessian
        H = solve(hessian)
        while (-t(gradient) %*% H %*% gradient < 0) {# eigen decomposition
          P = eigen(hessian)
          lambda = max(P$values)
          hessian =
            t(P$vectors) %*% (P$values - (lambda + tol) * diag(length(P$values))) %*% P$vectors
          H = solve(hessian)
        }
      }
    }
    ...
    #backtracking
    while (backtracking & fun(data,cur_theta)$loglink - fun(data,previous_theta)$loglink < 0 & trial < 2000) {
...
newton_update(fun = Poisson,previous_theta = c(7,2), data = dat)
```
...
naive_newton =
  function(fun,
    ...
    while (abs(f$loglink - prevlog) > tol && i < maxtiter) {
      i = i + 1
      prev_theta = cur_theta
      prevlog = f$loglink
      cur_theta = newton_update(fun, prev_theta,data)
      f = fun(data,cur_theta)
      result =
        rbind(result, tibble(
          iter = i,
          x_i = list(prev_theta),
...
```{r}
Beta_hat = naive_newton(Poisson,init_theta = c(7,2),data = dat)$theta
...
Bernoulli = 
  function(data,
    ...
    return(
      list(loglink = loglink*1e+4,
           fisher = fisher*1e+4,
           gradient  = gradient*1e+4,
           hessian = hessian*1e+4)
    )
  }

 Bernoulli(dat,rep(1,11))
...
```{r}
tibble(
  beta_hat = naive_newton(Bernoulli, init_theta = rep(0, 11), data = dat)$theta,
  beta = glm(diagnosis ~ ., data = breast, family = binomial())$coefficient %>% as.vector()
) %>% 
  knitr::kable()
```
...
```{r lasso}
lasso_update_fit =
  function(fun, data, theta_vec,lambda,... ) {
    iter = 0
    soft_threshold =
      function(beta, lambda)
        (abs(beta) > lambda) * (beta - sign(beta) * lambda)
    
    cur_result = fun(data, theta_vec)$loglink
    
    prev_result = -Inf
    
    result = tibble()
    
    while (abs(cur_result - prev_result) > tol && iter < maxiter) {
      iter = iter + 1
      
      for (i in 2:theta_vec) {
        #coordinate update
        theta_old = theta_vec[[i]]
        gradient_i = fun(data, theta_vec)$gradient[[i]]
        hessian_i = fun(data, theta_vec)$hessian[i, i]
        H_i = solve(hessian_i)
        theta_new = theta_old - H_i %*% gradient_i
        theta_vec[[i]] = soft_threshold(theta_new, lambda)
      }
      result = rbind(result,
                     tibble(
                       iteration = iter,
                       result = list(theta_vec),
                       L1 = fun(data, tehta_vec)$loglink
                     ))
    }
    return(list(theta = theta_vec, result = result)
  }

lasso_update = function(fun, data, theta_vec,
                        lambda = exp(seq(10, to = -10, 10)), ...) {
  ...
  for (lambda_i in lambda) {
    result = rbind(result,
                   tibble(
                     lamdba = lambda_i,
                     result = lasso_update_fit(fun, data, theta_vec, lambda)
                   ))
  }
  return(result)
}

lasso_update_fit(Bernoulli,data,theta_vec,lambda = 1)
```
