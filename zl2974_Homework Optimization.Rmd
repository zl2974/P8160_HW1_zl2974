...
The gradient is
$$\nabla l(Y;\beta) = (\frac{\partial}{\partial \beta_i}l(Y;\beta))=(\sum(y*\beta_i - \frac{\beta_i exp(X^T\beta)}{1+exp(X^T\beta)})$$
and the hessian is 
$$\nabla^2 l(Y;\beta)=(\sum \frac{\partial\beta_iy}{\partial\beta_j} +\frac{\beta_i*\beta_j*exp(X^T\beta)}{(1+exp(X^T\beta)^2)})$$
...
Bernoulli = 
  function(data,
    ...
    gradient = c(theta_vec*sum(Y-exp(X%*%theta_vec)/(1+exp(X%*%theta_vec))))
    
    hessian =
      expand.grid(beta_i = theta_vec,
                  beta_j = theta_vec) %>% 
      summarise(beta = beta_i*beta_j,
                beta = beta * sum(exp(X%*%theta_vec)/(1+exp(X%*%theta_vec))^2)) %>% 
      pull(beta)
    
    hessian = matrix(hessian,ncol = length(theta_vec))
    
    hessian = hessian + diag(theta_vec*sum(Y))
 ...
 Bernoulli(dat,rep(-2,10))$hessian %>% solve()
...
```{r}
r = 
  naive_newton(Bernoulli,init_theta = seq(-10,to = -1),data = dat)
```
