...
breast = 
  ...
  mutate(diagnosis = forcats::fct_relevel(diagnosis,"B"))
...
bfun =
  function(y, x, theta_vec) {
    if (is.factor(y))
      y = y %>% as.numeric() - 1
    if (exp(x %*% theta_vec)<1e-5){
      p_prev = exp(x %*% theta_vec) / (1 + 1e-5)
    } else  p_prev = exp(x %*% theta_vec) / (1 + exp(x %*% theta_vec)) #  row matrix
    ...
    while (abs(cur_result - prev_result) > tol
      ...
      for (i in 1:length(theta_vec)) {
        ...
        cur_theta =
         ...
         #soft-threshold
        print(str_c(lambda,i,theta_vec))
        if (i > 1) {
          cur_theta =
...
The result from glmnet shows that only radius, texture, smoothness and concave point and symetric are included in the model. The based odd is `r exp(coef(binomial_glmnet) %>% as.vector()[0])`, and increase a unit in `radius`, `texture`, `smoothness` and `concave point` and `symetric` will increase the odd of getting cancer`.
