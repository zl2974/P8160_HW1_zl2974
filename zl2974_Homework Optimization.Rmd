...
gfun = function(y,x,theta_vec){
  ...
  loglink = -sum(w *(y - x %*% theta_vec) ^ 2) / (2 * length(y))
  return(
...
tibble(glm = glm(Y_g~X_g)$coefficient,
       ...
       Qlasso_0lambda = Qlasso(Y_g,X_g,gfun),
       Qlasso_01lambda = Qlasso(Y_g,X_g,gfun,1),
       glmnet_01lambda = coef(glmnet(X_g,Y_g,lambda = 1)) %>% as.vector()
       )

tibble(glm = glm(Y~X,family = binomial())$coefficient,
       glmnet_0lambda = coef(glmnet(X,Y,"binomial",lambda = 0)) %>% as.vector(),
       Qlasso_0lambda = Qlasso(Y,X,qfun),
       Qlasso_01lambda = Qlasso(Y,X,qfun,3),
